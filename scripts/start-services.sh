...
# 日志文件
LOG_FILE="$PROJECT_ROOT/services-start.log"
ERROR_LOG_FILE="$PROJECT_ROOT/services-error.log"
...
check_env_files() {
    ...
    for service_dir in "$SERVICES_DIR"/*; do
        if [ -d "$service_dir" ]; then
            ...
            if [ ! -f "$env_file" ]; then
                print_warning "服务 $service_name 缺少 .env 文件"
                missing_env_files+=("$service_name")
            else
                print_success "服务 $service_name 环境文件检查通过"
            fi
        fi
    done
    
    if [ ${#missing_env_files[@]} -gt 0 ]; then
        print_warning "以下服务缺少环境变量文件: ${missing_env_files[*]}"
        print_info "请确保环境变量配置正确"
    fi
...
# 主启动函数
start_all_services() {
    print_header "开始启动Services服务"
    
    # 定义服务启动顺序（基础服务优先）
    local services=(
        "auth:auth:docker-compose.yaml"
        "wuji:wuji:docker-compose.yaml"
        "sast:sast:docker-compose.yaml"
        "web:web:docker-compose.yaml"
    )
...
show_successful_services_status() {
    ...
    for service_name in "${success_services[@]}"; do
        ...
        case $service_name in
            "auth")
                show_service_status "$SERVICES_DIR/auth" "$service_name" "docker-compose.yaml"
                ;;
            "wuji")
                show_service_status "$SERVICES_DIR/wuji" "$service_name" "docker-compose.yaml"
                ;;
            "sast")
                show_service_status "$SERVICES_DIR/sast" "$service_name" "docker-compose.yaml"
                ;;
            "web")
                show_service_status "$SERVICES_DIR/web" "$service_name" "docker-compose.yaml"
                ;;
        esac
        echo ""
    done
